...
def load_systematic_moving_heads():
    ...
    mh_fixtures = {
        fid: fixture
        for fid, fixture in data["fixtures"].items()
        if fixture["light_type"] == "moving_head"
    }

    print(
        f"🔍 Loaded {len(mh_fixtures)} Moving Head fixtures from systematic structure"
    )
...
def create_systematic_moving_head_show(capabilities, output_path: str):
    ...
    for bass_time in bass_times:
        if bass_time < song_duration - 5:
            ...
            dramatic_colors = [
                ...
                if c in ["red", "blue", "purple", "white_cool"]
            ]
            if dramatic_colors:
    ...
    for ambient_start in [5, 35, 70, 100, 130, 160, 190, 220]:
        if ambient_start < song_duration - 20:
            # Jemné barvy
            gentle_colors = [
                c for c in available_colors if c in ["blue", "green", "cyan", "purple"]
            ]
            if gentle_colors:
    ...
    print("   Effects: Sweeps, Chases, Bass responses, Ambient")
...
if __name__ == "__main__":
    ...
    if capabilities and capabilities["colors"]:
        ...
        create_systematic_moving_head_show(
            capabilities, "Kokoti_SYSTEMATIC_MOVING_HEADS.tml"
        )
