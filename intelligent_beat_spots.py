...
def load_systematic_structure():
    ...
    print("📁 Loaded systematic structure:")
...
def create_intelligent_beat_timeline(output_path: str):
    ...
    led_wall_fixtures = [
        fid for fid, fixture in fixtures.items() if fixture["light_type"] == "led_wall"
    ]
    ...
    for beat_time_raw in [
        i * beat_interval for i in range(int(song_duration / beat_interval))
    ]:
        ...
        scene_file = get_systematic_scene_path(
            fixtures, fixture_id, color_name, "static"
        )
        ...
        if beat_counter % 50 == 0:
            print(
                f"  Beat {beat_counter}: Fixture {fixture_id[-4:]}, Color {color_name}"
            )
    ...
    print("   Using systematic structure with precise positioning")
