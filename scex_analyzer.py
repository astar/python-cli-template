                            ...
                            for fixture_in_step in step.findall('.//Fixture'):
                                if fixture_in_step.get('id') == fixture_id:
                                    for channel in fixture_in_step.findall('Channel'):
                                        ...
                                        if channel_name:
                                            fixtures_db[fixture_id]['channels'].add(channel_name)
...
def analyze_fixture_patterns(fixtures_db):
    ...
    for fixture_id, info in fixtures_db.items():
        ...
        print(f"      Files: {info['files'][:3]}{'...' if len(info['files']) > 3 else ''}")
        print()

def load_sauna_layout():
    """Načte layout sauny z fixtures.ini."""

    fixtures_ini = Path("../auto-dmx/dmx/Infinit Maximus - new admin/3DView/fixtures.ini")
    ...
    if fixtures_ini.exists():
        ...
        with open(fixtures_ini, 'r', encoding='utf-8') as f:
            content = f.read()
    ...
    if other_ini.exists():
        ...
        with open(other_ini, 'r', encoding='utf-8') as f:
            content = f.read()
            for line in content.split('\n'):
...
def propose_systematic_structure(fixtures_db, layout):
    ...
    print(f"\n❌ Current Problems:")
    print(f"   • Inconsistent naming (MH_oven_blue vs LED_walls_1_red)")
    print(f"   • Missing fixtures (only some LED_walls have individual files)")
    print(f"   • No position-based organization")
    print(f"   • Manual creation = errors and gaps")

    # Propose solution
    print(f"\n✅ Proposed Systematic Structure:")
    print(f"   1. 🏗️  POSITION-BASED: Use 3D coordinates from layout")
    print(f"   2. 🎨 COLOR-BASED: Systematic color palette")
    print(f"   3. 🎛️  EFFECT-BASED: Systematic effect types")
    print(f"   4. 🤖 AUTO-GENERATED: Generate all combinations programmatically")

    print(f"\n📁 New Directory Structure:")
    print(f"   scenes/")
    print(f"   ├── generated/")
    print(f"   │   ├── single/          # Individual fixtures")
    print(f"   │   │   ├── led_wall_1/")
    print(f"   │   │   │   ├── static/  # Static colors")
    print(f"   │   │   │   │   ├── red.scex")
    print(f"   │   │   │   │   ├── blue.scex")
    print(f"   │   │   │   │   └── ...")
    print(f"   │   │   │   ├── dynamic/ # Dynamic effects")
    print(f"   │   │   │   │   ├── fade_red.scex")
    print(f"   │   │   │   │   ├── strobe_white.scex")
    print(f"   │   │   │   │   └── ...")
    print(f"   │   │   ├── led_wall_2/")
    print(f"   │   │   └── ...")
    print(f"   │   ├── groups/          # Grouped fixtures")
    print(f"   │   │   ├── walls_left/  # Left side walls")
    print(f"   │   │   ├── walls_right/")
    print(f"   │   │   ├── ceiling_center/")
    print(f"   │   │   └── ...")
    print(f"   │   └── patterns/        # Spatial patterns")
    print(f"   │       ├── circular/")
    print(f"   │       ├── wave/")
    print(f"   │       └── ...")

    print(f"\n🎨 Systematic Color Palette:")
    colors = ['red', 'green', 'blue', 'yellow', 'orange', 'purple', 'white_warm', 'white_cool', 'cyan', 'magenta']
    for i, color in enumerate(colors):
        print(f"   {i+1:2d}. {color}")

    print(f"\n⚡ Systematic Effect Types:")
    effects = ['static', 'fade_in', 'fade_out', 'strobe_slow', 'strobe_fast', 'pulse', 'chase', 'sweep']
...
if __name__ == "__main__":
    ...
    analysis_results = {
        'fixtures_db': {k: {**v, 'channels': list(v['channels'])} for k, v in fixtures_db.items()},
