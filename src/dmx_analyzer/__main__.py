...
def analyze(
    ...
    *,
    ...
    from .timeline_generator import TimelineGenerator, GenerationConfig
...
def spectacular(
    ...
    *,
    ...
    if dry_run:
        click.echo(f"Would create spectacular show for {audio_file} -> {output or 'auto-generated'}")
        return
    ...
    try:
        ...
        click.echo(f"✨ Spectacular timeline created with {len(timeline.events)} effects!")
...
def visualize(timeline_file: Path, audio_file: Path) -> None:
    ...
    try:
        ...
        click.echo(f"🎥 Starting real-time visualizer...")
        ...
        click.echo(f"❌ Visualization requires pygame: pip install pygame", err=True)
        click.echo(f"   Error: {e}", err=True)
        sys.exit(1)
    except Exception as e:
