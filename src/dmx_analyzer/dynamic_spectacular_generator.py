...
class DynamicSpectacularGenerator:
    ...
    def generate_spectacular_timeline(
        ...
        timeline.events = sorted(
            events, key=lambda e: self._time_to_seconds(e.start_time)
        )

        # Uložení
        timeline.save(output_path)
        logger.info(
            f"Dynamická timeline uložena: {output_path} ({len(timeline.events)} events)"
        )
    ...
    def _generate_beat_responsive_effects(
        self, analysis: dict[str, Any], duration: float
    ) -> list[DMXEvent]:
        ...
        for i, beat_time in enumerate(beat_times):
            ...
            if i % 4 == 0:
                # Flash všech bodovek
                effect_path = np.random.choice(
                    self.effect_fixtures["flash_all_ceiling"]
                )
                events.append(
                    DMXEvent(
    ...
    def _generate_strobe_effects(
        ...
        for i, (time, energy) in enumerate(zip(rms_times, high_energy, strict=False)):
            if (
                energy > high_threshold and time > last_strobe_time + 0.5
            ):  # Min 0.5s mezi stroby
                    ...
                    if strobe_time < duration - 1.0:
                        events.append(
                            DMXEvent(
                                ...
                                path=self.effect_fixtures["strobe_ceiling"][
                                    j % len(self.effect_fixtures["strobe_ceiling"])
                                ],
                                length="0:00:00.1",
                                speed=100,  # Maximální rychlost
                                speed_type=SpeedType.PERCENTAGE,
    ...
    def _generate_circular_waves(
        ...
        for i, beat_time in enumerate(beat_times):
            if i % 16 == 0 and beat_time < duration - 3.0:
                ...
                for j, fixture_path in enumerate(
                    wave_fixtures[:8]
                ):  # Prvních 8 bodovek
    ...
    def _generate_alternating_effects(
        self, analysis: dict[str, Any], duration: float
    ) -> list[DMXEvent]:
    ...
    def _generate_bass_explosions(
        ...
        for time, energy in zip(rms_times, bass_energy, strict=False):
            if (
                energy > bass_threshold and time > last_explosion_time + 1.5
            ):  # Min 1.5s mezi výbuchy
                # Výbušný efekt všech světel
                explosion_path = np.random.choice(
                    self.effect_fixtures["bass_explosions"]
                )
                events.append(
                    DMXEvent(
                        timeline_index=timeline_idx,
...
def create_dynamic_spectacular_timeline(
