...
class MusicDirectorVisualizer:
    ...
    def load_audio(self, audio_path: Path) -> None:
        ...
        self.spectrogram = np.abs(
            librosa.stft(self.audio_data, hop_length=hop_length, n_fft=n_fft)
        )
        ...
        logger.info(
            f"Audio analyzed: {self.duration:.1f}s, {len(self.music_features)} features"
        )
    ...
    def _analyze_music_features(self) -> None:
        ...
        centroid_times = librosa.frames_to_time(
            np.arange(len(centroid)), sr=self.sample_rate, hop_length=hop_length
        )
        ...
        beat_times = librosa.frames_to_time(
            beats, sr=self.sample_rate, hop_length=hop_length
        )
    ...
    def get_current_activation(self, current_time: float) -> dict[str, float]:
        ...
        for zone_name, zone in self.lighting_zones.items():
            ...
            if assigned_bands:
                ...
                if feature_key in self.music_features:
                    ...
                    if time_idx < len(feature.values):
                        ...
                        activation = max(
                            0.0, (value - feature.threshold) / feature.threshold
                        )
            ...
            activation *= self.mapping_rules["effects_settings"][
    ...
    def _get_frequency_band_energies(self, current_time: float) -> dict[str, float]:
        ...
        for band_name, (low_freq, high_freq) in self.mapping_rules[
            "frequency_bands"
        ].items():
    ...
    def _draw_spectrogram(self) -> None:
        ...
        title = self.font_medium.render(
            "SPEKTROGRAM - Real-time Analýza", True, self.text_color
        )
        ...
        if self.spectrogram is not None and self.times is not None:
            ...
            if end_idx > start_idx:
                ...
                spec_display = (
                    spec_display / np.max(spec_display)
                    if np.max(spec_display) > 0
                    else spec_display
                )

                # Vykreslení jako barevná mapa
                for y in range(
                    0, spec_display.shape[0], 4
                ):  # Skip every 4th frequency bin
                    for x in range(spec_display.shape[1]):
                        ...
                        # Pozice na obrazovce
                        screen_x = self.spectrogram_rect.x + (
                            x * self.spectrogram_rect.width // spec_display.shape[1]
                        )
                        screen_y = self.spectrogram_rect.bottom - (
                            y * self.spectrogram_rect.height // spec_display.shape[0]
                        )
        ...
        if self.duration > 0:
            time_pos = (
                self.current_time - max(0, self.current_time - 30)
            ) / 60  # Normalize to window
            marker_x = self.spectrogram_rect.x + int(
                time_pos * self.spectrogram_rect.width
            )
            pygame.draw.line(
                self.screen,
                (255, 255, 255),
                (marker_x, self.spectrogram_rect.y),
                (marker_x, self.spectrogram_rect.bottom),
                2,
            )
    ...
    def _draw_sauna_3d(self) -> None:
        ...
        # Title
        title = self.font_medium.render(
            "SAUNA - Zóny Aktivace & Fyzické Efekty", True, self.text_color
        )
        ...
        zone_rects = {
            "FLOOR_BASS": pygame.Rect(
                self.sauna_3d_rect.x + 220, self.sauna_3d_rect.y + 320, 60, 30
            ),
            "BENCHES_RHYTHM": [
                pygame.Rect(
                    self.sauna_3d_rect.x + 50, self.sauna_3d_rect.y + 280, 400, 20
                ),  # Bottom bench
                pygame.Rect(
                    self.sauna_3d_rect.x + 80, self.sauna_3d_rect.y + 250, 340, 20
                ),  # Mid bench
                pygame.Rect(
                    self.sauna_3d_rect.x + 110, self.sauna_3d_rect.y + 220, 280, 20
                ),  # Top bench
            ],
            "WALLS_LOW_MID": [
                pygame.Rect(
                    self.sauna_3d_rect.x + 20, self.sauna_3d_rect.y + 150, 15, 100
                ),  # Left wall
                pygame.Rect(
                    self.sauna_3d_rect.x + 465, self.sauna_3d_rect.y + 150, 15, 100
                ),  # Right wall
            ],
            "WALLS_HIGH_MID": [
                pygame.Rect(
                    self.sauna_3d_rect.x + 80, self.sauna_3d_rect.y + 40, 340, 15
                ),  # Top wall
                pygame.Rect(
                    self.sauna_3d_rect.x + 80, self.sauna_3d_rect.y + 360, 340, 15
                ),  # Bottom wall
            ],
            "CEILING_HARMONY": pygame.Rect(
                self.sauna_3d_rect.x + 80, self.sauna_3d_rect.y + 80, 340, 120
            ),
            "CEILING_BRILLIANCE": pygame.Rect(
                self.sauna_3d_rect.x + 180, self.sauna_3d_rect.y + 130, 140, 60
            ),
            "MOVING_DYNAMICS": [
                pygame.Rect(
                    self.sauna_3d_rect.x + 150, self.sauna_3d_rect.y + 100, 20, 20
                ),
                pygame.Rect(
                    self.sauna_3d_rect.x + 330, self.sauna_3d_rect.y + 100, 20, 20
                ),
                pygame.Rect(
                    self.sauna_3d_rect.x + 240, self.sauna_3d_rect.y + 120, 20, 20
                ),
                pygame.Rect(
                    self.sauna_3d_rect.x + 200, self.sauna_3d_rect.y + 180, 20, 20
                ),
                pygame.Rect(
                    self.sauna_3d_rect.x + 280, self.sauna_3d_rect.y + 180, 20, 20
                ),
            ],
            "UV_SPECIAL": pygame.Rect(
                self.sauna_3d_rect.x + 240, self.sauna_3d_rect.y + 160, 20, 20
            ),
        }
    ...
    def _draw_sauna_background(self) -> None:
        ...
        bench_positions = [
            # Bottom bench
            [
                real_3d_to_2d(-3.8, -3.6),
                real_3d_to_2d(3.8, -3.6),
                real_3d_to_2d(3.8, -3.0),
                real_3d_to_2d(-3.8, -3.0),
            ],
            # Mid bench
            [
                real_3d_to_2d(-3.5, -2.5),
                real_3d_to_2d(3.5, -2.5),
                real_3d_to_2d(3.5, -1.9),
                real_3d_to_2d(-3.5, -1.9),
            ],
            # Top bench
            [
                real_3d_to_2d(-3.2, -1.4),
                real_3d_to_2d(3.2, -1.4),
                real_3d_to_2d(3.2, -0.8),
                real_3d_to_2d(-3.2, -0.8),
            ],
        ]
    ...
    def _draw_fixture_positions(self, transform_func) -> None:
        ...
        wall_spots_3d = [
            ...
            (-2.02, 2.55),
            (2.02, 2.55),
            (3.21, 2.55),  # Horní stěna
            (-3.21, -2.60),
            (-2.02, -2.60),
            (2.02, -2.60),
            (3.21, -2.60),  # Dolní stěna
        ]
        for i, (x, z) in enumerate(wall_spots_3d):
            pos = transform_func(x, z)
            pygame.draw.rect(
                self.screen, (100, 100, 100), (pos[0] - 8, pos[1] - 8, 16, 16), 1
            )
    ...
    def _draw_geographical_effects(self, activations: dict[str, float]) -> None:
        ...
        if (
            bass_activation > 0.3
            and self.mapping_rules["effects_settings"]["bass_waves_enabled"]
        ):
            for i in range(3):
                ...
                if alpha > 0:
                    ...
                    pygame.draw.circle(
                        wave_surface, color_with_alpha, (radius, radius), radius, 3
                    )
                    self.screen.blit(
                        wave_surface, (center_x - radius, center_y - radius)
                    )

        # Rhythm pulses from benches - horizontal waves (if enabled)
        rhythm_activation = activations.get("BENCHES_RHYTHM", 0.0)
        if (
            rhythm_activation > 0.2
            and self.mapping_rules["effects_settings"]["rhythm_pulses_enabled"]
        ):
            ...
            for y_offset in [-40, 0, 40]:  # Three bench levels
                ...
                points = []
                for x in range(
                    self.sauna_3d_rect.left + 80, self.sauna_3d_rect.right - 80, 5
                ):
                    wave_offset = int(
                        wave_height
                        * math.sin(
                            (x - self.sauna_3d_rect.left) * 0.02 + self.current_time * 5
                        )
                    )
                ...
                if len(points) > 1:
                    ...
                    wave_surface = pygame.Surface(
                        self.sauna_3d_rect.size, pygame.SRCALPHA
                    )
        ...
        if (
            brilliance_activation > 0.4
            and self.mapping_rules["effects_settings"]["brilliance_sparkles_enabled"]
        ):
            import random
            ...
            for _ in range(num_sparkles):
                spark_x = random.randint(
                    self.sauna_3d_rect.x + 100, self.sauna_3d_rect.right - 100
                )
                spark_y = random.randint(
                    self.sauna_3d_rect.y + 80, self.sauna_3d_rect.y + 200
                )
                ...
                self.screen.blit(
                    spark_surface, (spark_x - spark_size, spark_y - spark_size)
                )

        # Moving heads - rotating beams (if enabled)
        moving_activation = activations.get("MOVING_DYNAMICS", 0.0)
        if (
            moving_activation > 0.3
            and self.mapping_rules["effects_settings"]["moving_beams_enabled"]
        ):
            ...
            for i, (mh_x, mh_y) in enumerate(mh_positions):
                ...
                pygame.draw.line(
                    beam_surface, color_with_alpha, (mh_x, mh_y), (end_x, end_y), 4
                )
    ...
    def _draw_zone_indicators(self, activations: dict[str, float]) -> None:
        ...
        zone_rects = {
            ...
            "BENCHES_RHYTHM": [
                pygame.Rect(
                    self.sauna_3d_rect.x + 70, self.sauna_3d_rect.y + 280, 360, 15
                ),  # Bottom bench
                pygame.Rect(
                    self.sauna_3d_rect.x + 90, self.sauna_3d_rect.y + 250, 320, 15
                ),  # Mid bench
                pygame.Rect(
                    self.sauna_3d_rect.x + 110, self.sauna_3d_rect.y + 220, 280, 15
                ),  # Top bench
            ],
            "WALLS_LOW_MID": [
                pygame.Rect(
                    self.sauna_3d_rect.x + 50, self.sauna_3d_rect.y + 150, 15, 100
                ),  # Left wall
                pygame.Rect(
                    self.sauna_3d_rect.x + 435, self.sauna_3d_rect.y + 150, 15, 100
                ),  # Right wall
            ],
            "WALLS_HIGH_MID": [
                pygame.Rect(
                    self.sauna_3d_rect.x + 70, self.sauna_3d_rect.y + 50, 360, 15
                ),  # Top wall
                pygame.Rect(
                    self.sauna_3d_rect.x + 70, self.sauna_3d_rect.y + 335, 360, 15
                ),  # Bottom wall
            ],
            "CEILING_HARMONY": pygame.Rect(
                self.sauna_3d_rect.x + 100, self.sauna_3d_rect.y + 80, 300, 120
            ),
            "CEILING_BRILLIANCE": pygame.Rect(
                self.sauna_3d_rect.x + 180, self.sauna_3d_rect.y + 130, 140, 60
            ),
    ...
    def _draw_waveforms(self) -> None:
        ...
        # Title
        title = self.font_medium.render(
            "FREKVENČNÍ PÁSMA - Live Monitoring", True, self.text_color
        )
        ...
        if self.music_features:
            ...
            for i, (feature_name, color) in enumerate(
                zip(features_to_show, colors, strict=False)
            ):
                if feature_name in self.music_features:
                    ...
                    if end_idx > start_idx:
                        ...
                        if len(values_slice) > 0:
                            ...
                            # Vykreslení křivky
                            y_offset = self.waveforms_rect.y + (i + 1) * (
                                self.waveforms_rect.height // 5
                            )
                            wave_height = self.waveforms_rect.height // 6

                            points = []
                            for j, (t, v) in enumerate(
                                zip(times_slice, values_norm, strict=False)
                            ):
                                x = self.waveforms_rect.x + int(
                                    (t - time_start)
                                    / (time_end - time_start)
                                    * self.waveforms_rect.width
                                )
                            ...
                            self.screen.blit(
                                label,
                                (
                                    self.waveforms_rect.x + 5,
                                    y_offset - wave_height - 15,
                                ),
                            )
        ...
        if self.duration > 0:
            ...
            pygame.draw.line(
                ...
                (marker_x, self.waveforms_rect.bottom),
                2,
            )
    ...
    def _draw_zone_controls(self) -> None:
        ...
        self.screen.blit(
            title, (self.zones_control_rect.x, self.zones_control_rect.y - 25)
        )
        ...
        for i, (zone_name, zone) in enumerate(self.lighting_zones.items()):
            ...
            # Zone name
            zone_text = self.font_small.render(
                f"{zone.name[:20]}", True, self.text_color
            )
            self.screen.blit(zone_text, (self.zones_control_rect.x + 10, y_offset))

            # Activation bar
            bar_rect = pygame.Rect(self.zones_control_rect.x + 200, y_offset, 200, 15)
            ...
            perc_text = self.font_small.render(
                f"{activation:.1%}", True, self.text_color
            )
    ...
    def _draw_timeline(self) -> None:
        ...
        # Title
        title = self.font_medium.render(
            "TIMELINE - Hudební Struktura", True, self.text_color
        )
        ...
        if self.duration > 0:
            ...
            for t in np.arange(0, self.duration, 30):  # Every 30 seconds
                x_pos = self.timeline_rect.x + int(
                    (t / self.duration) * self.timeline_rect.width
                )
                pygame.draw.line(
                    self.screen,
                    (100, 100, 100),
                    (x_pos, self.timeline_rect.y),
                    (x_pos, self.timeline_rect.bottom),
                    1,
                )

                # Time label
                time_text = self.font_small.render(
                    f"{int(t // 60)}:{int(t % 60):02d}", True, (150, 150, 150)
                )
            ...
            if "beats" in self.music_features:
                ...
                for i, (beat_time, beat_strength) in enumerate(
                    zip(beats_feature.times, beats_feature.values, strict=False)
                ):
                    if beat_strength > 0.5:  # Only strong beats
                        x_pos = self.timeline_rect.x + int(
                            (beat_time / self.duration) * self.timeline_rect.width
                        )
            ...
            # Current time marker
            current_x = self.timeline_rect.x + int(
                (self.current_time / self.duration) * self.timeline_rect.width
            )
            pygame.draw.line(
                self.screen,
                (255, 255, 255),
                (current_x, self.timeline_rect.y),
                (current_x, self.timeline_rect.bottom),
                3,
            )
    ...
    def _draw_controls_info(self) -> None:
        ...
        if self.music_features:
            ...
            effects_count = sum(
                1
                for setting, enabled in self.mapping_rules["effects_settings"].items()
...
def run_music_director(audio_path: Path) -> None:
    ...
    print("🎼 Starting Music Director Visualizer...")
